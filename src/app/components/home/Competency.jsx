...
const CoreCompetencies = () => {
  ...
  return (
    <section className="relative pb-16 text-black font-sfth">
      {/* subtle gradient behind the grid */}
      <div className="absolute inset-0 pointer-events-none" />

      <div className="relative 2xl:max-w-7xl max-w-6xl mx-auto px-6 lg:px-8">
        {/* Section Heading */}
            <h3 className="font-jost md:text-5xl text-2xl font-bold mb-5 pb-2 textMask_br text-center translate-y-[-3rem]">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 translate-y-[-1rem]">
          {competencies.map(({ title, description, Icon }) => (
            <div
              key={title}
              className="group bg-white p-6 rounded-2xl shadow-lg hover:shadow-xl relative hover:scale-90 ease-in duration-200 transition-all"
            >
              {/* Icon container */}
              ...
              <p className="font-sfth text-gray-800 2xl:text-base text-sm leading-relaxed" style={{maskImage:'linear-gradient(135deg, black 60%, #00000077)'}}>
                {description}
              </p>
              ...
              <button className="mt-6 w-full inline-flex items-center justify-between border border-primary-blue rounded-full py-2  pr-2 pl-4 overflow-hidden group transition">
                <span className="font-sfbo text-black 2xl:text-base text-sm">View Details</span>
